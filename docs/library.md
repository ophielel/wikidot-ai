...
TypeScript library for working with Wikidot sites. This is the underlying SDK used
by the `wikidot-ai` agent. If you only want to drive the agent from natural language,
read the [main README](../README.md) instead.

## Overview

wikidot-ts is a TypeScript library for working with the Wikidot API. It supports major
Wikidot features including page retrieval and editing, forum operations, private
messages, and user information retrieval.
...
This library uses the Result type from `neverthrow`. All async methods return
`WikidotResultAsync<T>`. Check success with `isOk()`, then access the value with
`.value`.
...
wikidot-ts is a port of wikidot.py, with some differences to follow TypeScript
conventions.

### Property to Method Conversion

Python properties using the `@property` decorator are implemented as getter methods in
TypeScript.
...
The library is built and tested with Bun:
